...
router.get('/logs', authorizeRoles('admin', 'teacher'), async (req, res) => {
  try {
...
router.post('/scan', authorizeRoles('admin', 'teacher'), async (req, res) => {
...
router.post('/enroll', authorizeRoles('admin', 'teacher'), async (req, res) => {
...
router.get('/today-summary', authorizeRoles('admin', 'teacher'), async (req, res) => {
  try {
    const today = new Date().toISOString().slice(0, 10);
    const query = `
